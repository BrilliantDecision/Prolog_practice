...
%«јƒј„ј 9
% примечание - пустые строки не учитываютс€ и на выполнение программы не
% вли€ют
pr5_9:-
    ...
    write("—амое частое слово:"),write(Word),nl,
    write("„астота:"),write(Max),nl.
...
task_9(InputList,Word,Max):-
    beau_list(InputList,[],NewList),% удал€ем enter's
    all_make_list_words(NewList,[],OutWord), % удал€ем пробелы и составл€ем список слов
    count_one_word(OutWord,[],[],WordList,FrequencyList), %—оставл€ем список слов и этому списку - список частоты встречаемости соответственно
    max_num(Max, 0, FrequencyList, 0, 0,Index), % максимальна€ частота
...
%сравниваем элементы в списках на равных позици€х
check_lists([],[]):-!.
